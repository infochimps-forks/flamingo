  ...
  module Daemon
    class WaderProcess < ChildProcess
      ...
      def run
        ...
        auth     = case
                   when config.oauth.present?       then { :oauth => config.oauth }
                   when config.username.present? then { :auth  => "#{config.username}:#{config.password}" }
                   else exit(EXIT_AUTHENTICATION) end

        @wader = Flamingo::Wader.new(stream, auth)
